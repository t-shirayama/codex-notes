...
新規記事作成だけでなく、既存記事を参考元で再確認し、本文・`最終ファクトチェック`・記事台帳を更新する用途にも使う。
...
- 1ファイル1トピックを守る
- 冒頭に `種別`、`参考元`、`最終ファクトチェック`、`確認メモ` を入れる
- 末尾に `参考元` セクションを残す
...
- `checked` は「記録時点で参考元と照合済み」を意味する
- 台帳を更新する前に、本文側の参考元・日付・確認メモと矛盾しないか確認する

### 7. ファクトチェック更新フロー

- 対象記事と参考元を見比べる
- 本文の論点、見出し、注意書きが参考元とずれていないか確認する
...
- 編集権限がない、または直接編集できない環境では、ファイルを更新したとは書かず、対象ファイルごとの差分案として提示する
- ファイルを直接更新できない場合は、保存先、差分案、台帳更新案を分けて提示する
- 記事台帳を更新する場合は、本文側の `最終ファクトチェック`、`確認メモ`、参考元リンクと矛盾しないことを確認する
- 本文と記事台帳を両方更新する場合は、どちらか片方だけが新しい状態にならないようにする
...
```txt
この official 記事を参考元で再確認して、差分があれば本文とファクトチェック日を更新して。
article-registry.md のメモも更新して。
```

```txt
この curation 記事の参考元を見直して、古くなった主張があれば修正案を出して。
